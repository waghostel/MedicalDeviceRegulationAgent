...
- [ ] 1. Fix MSW Mock Server Setup and Integration Test Infrastructure
...
- [ ] 3. Fix Project Form Toast Notifications and Error Handling
  ...
  - **Code snippet**:
    ...
    const handleSubmit = async (data) => {
      try {
        await createProject(data);
        toast.success('Project Created', `Project "${data.name}" created successfully`);
        onClose();
      } catch (error) {
        if (error.status === 401) {
    ...
    const ResponsiveProjectCard = ({ project }) => {
      ...
      return (
        <Card className={cn(
          ...
          <div className={cn(
            'flex',
            isMobile ? 'flex-col space-y-2' : 'flex-row items-center justify-between'
          )}>
            <h3 className={isMobile ? 'text-lg' : 'text-xl'}>
              {project.name}
            </h3>
            {isMobile ? (
