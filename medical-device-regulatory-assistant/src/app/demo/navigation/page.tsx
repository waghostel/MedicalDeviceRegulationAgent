...
export default function NavigationDemoPage() {
  ...
  const handleFileSelect = (file: FileNode) => {
    ...
    const timestamp = new Date().toLocaleTimeString();
    setActionLog((prev) => [
      `[${timestamp}] File selected: ${file.name}`,
      ...prev.slice(0, 9),
    ]);
  };
  ...
  const handleFileCreate = (
    ...
    const timestamp = new Date().toLocaleTimeString();
    setActionLog((prev) => [
      `[${timestamp}] Create ${type} in ${parentId || 'root'}`,
      ...prev.slice(0, 9),
    ]);
  };
  ...
  const handleFileRename = (fileId: string, newName: string) => {
    const timestamp = new Date().toLocaleTimeString();
    setActionLog((prev) => [
      `[${timestamp}] Rename file ${fileId} to ${newName}`,
      ...prev.slice(0, 9),
    ]);
  };
  ...
  const handleFileUpload = (files: FileList, parentId: string | null) => {
    ...
    const fileNames = Array.from(files)
      ...
      .join(', ');
    setActionLog((prev) => [
      `[${timestamp}] Upload files: ${fileNames}`,
      ...prev.slice(0, 9),
    ]);
  };
      ...
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold">
            Navigation & Quick Actions Demo
          </h1>
          <p className="text-muted-foreground mt-2">
            Test the navigation components, keyboard shortcuts, and file
              ...
              <div className="space-y-1 max-h-48 overflow-y-auto">
                {actionLog.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    No actions yet. Try using the quick actions or keyboard
                    shortcuts!
                  </p>
                ) : (
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-4">
              These buttons simulate the same actions as the quick actions
