...
export function useAgentExecution(options: UseAgentExecutionOptions = {}) {
  ...
  const startStatusMonitoring = useCallback((sessionId: string) => {
  ...
  const executeTask = useCallback(async (
    ...
    try {
      ...
      const completedStatus: AgentExecutionStatus = {
        ...
        message: result.status === 'completed' ? 'Task completed successfully' : 'Task in progress...',
  ...
  }, [enableRealTimeUpdates, startStatusMonitoring, onStatusUpdate, onComplete, onError]);
  ...
  // Get session status
  const getSessionStatus = useCallback(async (sessionId: string): Promise<AgentExecutionStatus> => {
    try {
