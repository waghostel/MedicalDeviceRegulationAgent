...
export function useClassification({ 
  ...
  const loadClassification = useCallback(async (showLoading = true) => {
    ...
    try {
      const classification = await dashboardService.getClassification(projectId);
  ...
  const startClassification = useCallback(async () => {
    try {
      ...
      toast({
        ...
        description: result.message || 'Device classification analysis has been initiated.',
      });
      ...
      toast({
        ...
        description: error.message || 'Failed to start classification analysis.',
        variant: 'destructive',
      });
    }
