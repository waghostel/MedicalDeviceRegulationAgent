...
export function useProjects(initialFilters: ProjectSearchFilters = {}) {
  ...
  const loadProjects = useCallback(async (filters: ProjectSearchFilters = state.filters) => {
  ...
  const filterProjects = useCallback(async (filters: Partial<ProjectSearchFilters>) => {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error.message || 'An error occurred while loading more projects.',
        variant: 'destructive',
      });
    }
...
export function useProject(projectId: number | null) {
  ...
  const updateProject = useCallback(async (projectData: ProjectUpdateRequest): Promise<boolean> => {
    ...
    try {
      ...
      setState(prev => ({
        ...prev,
        project: prev.project ? { ...prev.project, ...projectData } : undefined,
      }));
  ...
  const exportProject = useCallback(async (format: 'json' | 'pdf' = 'json') => {
    if (!projectId) return;
    ...
    try {
      ...
      toast({
        title: 'Export Failed',
        description: error.message || 'An error occurred while exporting the project.',
        variant: 'destructive',
      });
    }
  }, [projectId]);
