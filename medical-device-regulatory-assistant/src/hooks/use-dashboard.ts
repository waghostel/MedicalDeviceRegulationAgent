...
export function useDashboard({ 
  ...
  const exportDashboard = useCallback(async (format: 'json' | 'pdf' = 'json') => {
    try {
      const blob = await projectService.exportProject(parseInt(projectId), format);
  ...
  const selectPredicate = useCallback(async (predicate: any) => {
    try {
