...
import { FeatureFlagManager, MIGRATION_FLAGS, FeatureFlag } from './feature-flag-system';
...
export interface MigrationCondition {
    type: 'error_rate' | 'response_time' | 'success_rate' | 'user_feedback' | 'custom';
...
export class MigrationAutomationManager {
    ...
    private evaluateRules(healthCheck: MigrationHealthCheck): void {
        const applicableRules = Array.from(this.rules.values())
            .filter(rule => rule.enabled && (rule.flagKey === '*' || rule.flagKey === healthCheck.flagKey))
    ...
    private evaluateConditions(conditions: MigrationCondition[], healthCheck: MigrationHealthCheck): boolean {
    ...
    private async executeActions(actions: MigrationAction[], healthCheck: MigrationHealthCheck, rule: MigrationRule): Promise<void> {
        for (const action of actions) {
            if (action.delay) {
                setTimeout(() => this.executeAction(action, healthCheck, rule), action.delay * 60 * 1000);
            } else {
    ...
    private async executeAction(action: MigrationAction, healthCheck: MigrationHealthCheck, rule: MigrationRule): Promise<void> {
        try {
            switch (action.type) {
                case 'rollback':
                    await this.performRollback(healthCheck.flagKey, action.parameters.reason);
                    break;
                ...
                case 'reduce_traffic':
                    await this.reduceTraffic(healthCheck.flagKey, action.parameters.percentage);
                    break;
    ...
    private async performRollback(flagKey: string, reason: string): Promise<void> {
    ...
    private async reduceTraffic(flagKey: string, percentage: number): Promise<void> {
    ...
    private createAlert(alert: MigrationAlert): void {
        ...
        // Log alert
        console.log(`Migration Alert [${alert.severity.toUpperCase()}]: ${alert.message}`, {
    ...
    public getHealthHistory(flagKey: string, limit: number = 50): MigrationHealthCheck[] {
        const checks = this.healthChecks.get(flagKey) || [];
        return checks.slice(-limit);
    }

    public getAlerts(flagKey?: string, severity?: string, limit: number = 100): MigrationAlert[] {
        let filteredAlerts = this.alerts;

        if (flagKey) {
            filteredAlerts = filteredAlerts.filter(alert => alert.flagKey === flagKey);
        }
    ...
    public generateHealthReport(flagKey: string): {
        ...
        if (checks.length === 0) {
            return {
                ...
                trends: { errorRateTrend: 'stable', responseTimeTrend: 'stable', successRateTrend: 'stable' },
                recommendations: ['No health data available']
            };
        }

        // Calculate averages
        const avgErrorRate = checks.reduce((sum, check) => sum + check.metrics.errorRate, 0) / checks.length;
        const avgResponseTime = checks.reduce((sum, check) => sum + check.metrics.responseTime, 0) / checks.length;
        const avgSuccessRate = checks.reduce((sum, check) => sum + check.metrics.successRate, 0) / checks.length;

        // Determine overall health
        let overall: 'healthy' | 'warning' | 'critical' = 'healthy';
        if (avgErrorRate > 0.05 || avgResponseTime > 2000 || avgSuccessRate < 0.95) {
            overall = 'critical';
        } else if (avgErrorRate > 0.02 || avgResponseTime > 1000 || avgSuccessRate < 0.98) {
            overall = 'warning';
        }
        ...
        const trends = {
            ...
            responseTimeTrend: this.calculateTrend(firstHalf, secondHalf, 'responseTime'),
            successRateTrend: this.calculateTrend(firstHalf, secondHalf, 'successRate', true)
        };

        // Generate recommendations
        const recommendations: string[] = [];
        if (avgErrorRate > 0.02) recommendations.push('Consider investigating error causes');
        if (avgResponseTime > 1000) recommendations.push('Performance optimization may be needed');
        if (avgSuccessRate < 0.98) recommendations.push('Review success rate metrics');
        if (trends.errorRateTrend === 'degrading') recommendations.push('Error rate is increasing');
        if (trends.responseTimeTrend === 'degrading') recommendations.push('Response time is degrading');
        if (trends.successRateTrend === 'degrading') recommendations.push('Success rate is declining');
    ...
    private calculateTrend(
        ...
        const firstAvg = firstHalf.reduce((sum, check) => sum + (check.metrics[metric] as number), 0) / firstHalf.length;
        const secondAvg = secondHalf.reduce((sum, check) => sum + (check.metrics[metric] as number), 0) / secondHalf.length;
    ...
    public startMonitoring(intervalMinutes: number = 5): void {
        ...
        console.log(`Migration monitoring started with ${intervalMinutes} minute intervals`);
    }
    ...
    private async performHealthChecks(): Promise<void> {
        ...
        for (const flag of activeFlags) {
            if (flag.key.startsWith('migrate_')) {
                ...
                if (healthCheck.metrics.errorRate > healthCheck.thresholds.maxErrorRate ||
                    healthCheck.metrics.responseTime > healthCheck.thresholds.maxResponseTime ||
                    healthCheck.metrics.successRate < healthCheck.thresholds.minSuccessRate) {
                    healthCheck.status = 'critical';
                    healthCheck.recommendation = 'rollback';
                } else if (healthCheck.metrics.errorRate > healthCheck.thresholds.maxErrorRate * 0.7 ||
                    healthCheck.metrics.responseTime > healthCheck.thresholds.maxResponseTime * 0.7 ||
                    healthCheck.metrics.successRate < healthCheck.thresholds.minSuccessRate + 0.02) {
