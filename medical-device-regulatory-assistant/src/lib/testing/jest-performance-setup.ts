  ...
  afterAll(() => {
    if (currentSuite) {
      try {
        ...
        console.log(
          `  Execution Time: ${(metrics.totalExecutionTime / 1000).toFixed(2)}s`
        );
        console.log(
          `  Tests: ${metrics.passedTests}/${metrics.totalTests} passed`
        );
        console.log(
        ...
        console.warn(
          `Failed to end suite tracking for ${currentSuite}:`,
          error
        );
      }
      currentSuite = null;
  ...
  afterEach(() => {
    if (currentTestId && currentSuite) {
      try {
        ...
        console.warn(
          `Failed to end test tracking for ${currentTestId}:`,
          error
        );
      }
      currentTestId = null;
...
function determineTestStatus(testState: any): 'passed' | 'failed' | 'skipped' {
  ...
  if (
    testState.currentTestName?.includes('skip') ||
...
export function performanceTest(
  ...
  return async () => {
    ...
    try {
      ...
      console.log(
        `⏱️  ${testName}: ${executionTime.toFixed(2)}ms, ${memoryUsage.toFixed(2)}MB`
      );
    }
  };
}
  ...
  return () => {
    ...
    afterAll(() => {
      ...
      if (options?.maxSuiteTime) {
        expect(metrics.totalExecutionTime).toBeLessThanOrEqual(
          options.maxSuiteTime
        );
      }
