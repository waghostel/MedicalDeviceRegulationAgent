...
import { TestFailureAnalyzer, testFailureAnalyzer } from '../TestFailureAnalyzer';
...
describe('TestFailureAnalyzer', () => {
  ...
  describe('analyzeFailure', () => {
    it('should analyze React 19 AggregateError correctly', () => {
      ...
      const report = testFailureAnalyzer.analyzeFailure('ProjectForm test', aggregateError);
      ...
      expect(report.suggestions).toContain('Wrap component in React19ErrorBoundary');
    });

    it('should analyze hook mock configuration errors', () => {
      const hookError = new Error('useToast is not a function');

      const report = testFailureAnalyzer.analyzeFailure('useToast hook test', hookError);
      ...
      expect(report.suggestions).toContain('Verify hook mock structure matches actual implementation');
    });

    it('should analyze component rendering errors', () => {
      const renderError = new Error('Cannot render component: missing required props');

      const report = testFailureAnalyzer.analyzeFailure('Component render test', renderError);
      ...
      expect(report.suggestions).toContain('Provide all required props to component');
    });

    it('should analyze timeout errors', () => {
      const timeoutError = new Error('Test timed out after 5000ms');

      const report = testFailureAnalyzer.analyzeFailure('Async test', timeoutError);
    ...
    it('should analyze assertion errors', () => {
      ...
      const report = testFailureAnalyzer.analyzeFailure('Assertion test', assertionError);
    ...
    it('should generate debugging steps for different failure types', () => {
      ...
      const report = testFailureAnalyzer.analyzeFailure('Enhanced form test', mockError);

      expect(report.debuggingSteps).toHaveLength(3);
      expect(report.debuggingSteps[0].description).toContain('Examine the error message');
      expect(report.debuggingSteps[1].description).toContain('Validate hook mock structure');
      expect(report.debuggingSteps[2].description).toContain('Run test in isolation');
    });
    ...
    it('should identify related files based on failure category', () => {
      ...
      expect(report.relatedFiles).toContain('src/lib/testing/enhanced-form-hook-mocks.ts');
      expect(report.relatedFiles).toContain('src/lib/testing/use-toast-mock.ts');
    ...
    it('should calculate confidence scores appropriately', () => {
      ...
      const knownReport = testFailureAnalyzer.analyzeFailure('Known error test', knownError);
      const unknownReport = testFailureAnalyzer.analyzeFailure('Unknown error test', unknownError);
  ...
  describe('generateTroubleshootingGuide', () => {
    ...
    it('should include rendering issues section when present', () => {
      ...
      const report = testFailureAnalyzer.analyzeFailure('Render test', renderError);
  ...
  describe('getFailureStatistics', () => {
    ...
    it('should calculate statistics correctly with multiple failures', () => {
      ...
      testFailureAnalyzer.analyzeFailure('Test 2', new AggregateError([], 'Aggregate error'));
    ...
    it('should limit recent failures to last 10', () => {
      ...
      for (let i = 0; i < 15; i++) {
        testFailureAnalyzer.analyzeFailure(`Test ${i}`, new Error(`Error ${i}`));
      }
  ...
  describe('error pattern recognition', () => {
    it('should recognize common error patterns', () => {
      const patterns = [
        { error: 'useToast is not a function', expectedCategory: 'HOOK_MOCK_CONFIGURATION' },
        { error: 'Cannot read property of undefined', expectedCategory: 'COMPONENT_RENDERING' },
        { error: 'AggregateError: Multiple errors', expectedCategory: 'REACT_19_COMPATIBILITY' },
        { error: 'Provider not found in context', expectedCategory: 'PROVIDER_SETUP' },
        { error: 'Test timeout exceeded', expectedCategory: 'PERFORMANCE_ISSUE' }
      ];

      patterns.forEach(({ error, expectedCategory }) => {
        const report = testFailureAnalyzer.analyzeFailure('Pattern test', new Error(error));
  ...
  describe('performance metrics', () => {
    it('should complete analysis within reasonable time', () => {
      ...
      testFailureAnalyzer.analyzeFailure('Performance test', new Error('Test error'));
    ...
    it('should handle large error messages efficiently', () => {
      ...
      const startTime = performance.now();
      const report = testFailureAnalyzer.analyzeFailure('Large error test', largeError);
      const duration = performance.now() - startTime;
  ...
  describe('edge cases', () => {
    it('should handle errors without stack traces', () => {
      ...
      const report = testFailureAnalyzer.analyzeFailure('No stack test', errorWithoutStack);
    ...
    it('should handle empty error messages', () => {
      ...
      const report = testFailureAnalyzer.analyzeFailure('Empty error test', emptyError);
    ...
    it('should handle context with component stack', () => {
      ...
      const report = testFailureAnalyzer.analyzeFailure('Context test', error, context);
