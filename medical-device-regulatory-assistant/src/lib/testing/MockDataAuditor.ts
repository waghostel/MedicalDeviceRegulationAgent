...
export class MockDataAuditor {
  ...
  constructor(
    srcPath: string = 'src',
    mockDataPath: string = 'src/lib/mock-data.ts'
  ) {
  ...
  public async scanComponents(): Promise<ComponentMockUsage[]> {
    ...
    for (const componentPath of components) {
      ...
      if (
        analysis.mockDataImports.length > 0 ||
        analysis.mockDataUsage.length > 0
      ) {
        results.push({
          componentPath: analysis.componentPath,
          mockDataUsed: analysis.mockDataUsage.map(
            (usage) => usage.functionName
          ),
    ...
    return results.sort((a, b) => {
      ...
      return (
        priorityOrder[b.migrationPriority] - priorityOrder[a.migrationPriority]
      );
    });
  }
   ...
   */
  public calculateMigrationPriority(
    ...
    const totalUsage = analysis.mockDataUsage.reduce(
  ...
  public async generateAuditReport(): Promise<AuditReport> {
    ...
    return {
      summary: {
        ...
        mockDataFunctions,
        migrationReadiness:
          this.calculateOverallMigrationReadiness(componentAnalysis),
      },
      componentAnalysis,
      dependencyGraph,
      migrationPlan,
  ...
  private generateMigrationPlan(
    components: ComponentMockUsage[]
  ): MigrationPlan {
    const phases: MigrationPhase[] = [];

    // Phase 1: Low priority, low risk components
    const phase1Components = components.filter(
      (c) => c.migrationPriority === 'low'
    );
    ...
    const phase2Components = components.filter(
      (c) => c.migrationPriority === 'medium'
    );
    ...
    const phase3Components = components.filter(
      (c) => c.migrationPriority === 'high'
    );
    if (phase3Components.length > 0) {
      phases.push({
        name: 'Phase 3: High-Priority Components',
        components: phase3Components.map((c) => c.componentPath),
        dependencies: [...phase1Components, ...phase2Components].map(
          (c) => c.componentPath
        ),
   ...
   */
  private calculateOverallMigrationReadiness(
    analyses: ComponentAnalysis[]
  ): number {
    if (analyses.length === 0) return 1.0;

    const totalScore = analyses.reduce(
      (sum, analysis) => sum + analysis.migrationReadiness.score,
      0
    );
  ...
  private generateRecommendations(analyses: ComponentAnalysis[]): string[] {
    ...
    const highRiskComponents = analyses.filter(
      (a) => a.migrationReadiness.riskLevel === 'high'
    );
    if (highRiskComponents.length > 0) {
      recommendations.add(
        'Focus on high-risk components first - add comprehensive tests'
      );
      recommendations.add(
        'Consider refactoring complex components before migration'
      );
    }
    ...
    if (noTestComponents.length > 0) {
      recommendations.add(
        `${noTestComponents.length} components lack tests - prioritize test coverage`
      );
    }
    ...
    if (heavyMockUsage.length > 0) {
      recommendations.add(
        'Components with heavy mock data usage should be refactored for easier migration'
      );
    }
