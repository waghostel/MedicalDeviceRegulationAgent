...
export interface ContextValidationError {
    type: 'missing_context' | 'invalid_value' | 'type_mismatch' | 'missing_method' | 'provider_not_found';
    message: string;
    expected?: any;
    actual?: any;
    path?: string;
...
export interface ContextValidationWarning {
    type: 'deprecated_method' | 'performance_concern' | 'missing_optional' | 'version_mismatch';
    message: string;
    suggestion?: string;
...
export class ContextMockValidator {
    ...
    validateContext<T = any>(
        ...
        try {
            ...
            const valueValidation = this.validateContextValue(contextName, expectedValue);
            errors.push(...valueValidation.errors);
            warnings.push(...valueValidation.warnings);

            // 3. Validate context methods
            const methodValidation = this.validateContextMethods(contextName, expectedValue);
            errors.push(...methodValidation.errors);
            warnings.push(...methodValidation.warnings);

            // 4. Perform render test
            if (Provider) {
                const renderValidation = this.validateContextRender(contextName, Provider, expectedValue);
                errors.push(...renderValidation.errors);
                warnings.push(...renderValidation.warnings);
            }

            // 5. Generate suggestions
            suggestions.push(...this.generateValidationSuggestions(contextName, errors, warnings));
    ...
    validateThemeContext(mockValue: ThemeContextValue): ContextValidationResult {
        return this.validateContext('ThemeContext', {} as Context<ThemeContextValue>, mockValue);
    }

    /**
     * Validate Session context specifically
     */
    validateSessionContext(mockValue: SessionContextValue): ContextValidationResult {
        return this.validateContext('SessionContext', {} as Context<SessionContextValue>, mockValue);
    }
    ...
    generateDebugInfo<T>(
        ...
        // Analyze methods
        const methodAnalysis = this.analyzeContextMethods(contextName, contextValue);
        ...
        if (Provider) {
            try {
                ...
                debugInfo.renderTest.error = error instanceof Error ? error.message : 'Unknown render error';
            }
        }
    ...
    validateAllContexts(): Record<string, ContextValidationResult> {
        ...
        const contextValidations = [
            {
                name: 'ToastContext',
                validator: () => this.validateToastContext(this.createMockToastContext()),
            },
            {
                name: 'FormContext',
            ...
            {
                name: 'ThemeContext',
                validator: () => this.validateThemeContext(this.createMockThemeContext()),
            },
            {
                name: 'SessionContext',
                validator: () => this.validateSessionContext(this.createMockSessionContext()),
            },
        ];
    ...
    getValidationStats(): {
        ...
        return {
            ...
            averageValidationTime: results.reduce((sum, r) => sum + r.validationTime, 0) / results.length || 0,
        };
    }
    ...
    private validateContextValue(contextName: string, value: any): {
        errors: ContextValidationError[];
        warnings: ContextValidationWarning[];
    } {
        const errors: ContextValidationError[] = [];
    ...
    private validateContextMethods(contextName: string, value: any): {
        errors: ContextValidationError[];
        warnings: ContextValidationWarning[];
    } {
        const errors: ContextValidationError[] = [];
    ...
    private validateContextRender(
        ...
        try {
            const TestComponent = () => React.createElement('div', { 'data-testid': 'context-test' }, 'Test');
            const WrappedComponent = () =>
                React.createElement(
                    Provider,
                    { value },
    ...
    private generateValidationSuggestions(
        ...
        if (errors.some(e => e.type === 'missing_context')) {
            suggestions.push(`Ensure ${contextName} is properly initialized in test setup`);
        }

        if (errors.some(e => e.type === 'missing_method')) {
            suggestions.push(`Add missing methods to ${contextName} mock implementation`);
        }
        ...
        if (errors.some(e => e.type === 'provider_not_found')) {
            suggestions.push(`Verify ${contextName} provider is properly imported and mocked`);
        }

        if (warnings.some(w => w.type === 'performance_concern')) {
            suggestions.push(`Consider optimizing ${contextName} mock for better test performance`);
        }
    ...
    private getExpectedMethods(contextName: string): {
        ...
        const methodMap: Record<string, { required: string[]; optional: string[] }> = {
            ToastContext: {
    ...
    private analyzeContextMethods(contextName: string, value: any): {
        missing: string[];
        extra: string[];
    } {
 ...
 */
export function createContextMockValidator(config?: Partial<ContextValidationConfig>): ContextMockValidator {
