...
class JestHealthReporter {
  ...
  onTestResult(test, testResult, aggregatedResult) {
    ...
    testResult.testResults.forEach(result => {
      const testRecord = {
        ...
        error: result.failureMessages.length > 0 ? result.failureMessages.join('\n') : undefined,
        timestamp: Date.now(),
  ...
  extractWarnings(testResult) {
    ...
    if (testResult.console && testResult.console.length > 0) {
      const warningLogs = testResult.console.filter(log => 
        log.type === 'warn' || log.message.includes('Warning')
      );
  ...
  logHealthSummary(healthReport) {
    ...
    console.log(`${statusIcon} Overall Status: ${healthReport.status.toUpperCase()}`);
    
    // Key metrics
    console.log('\n📈 Key Metrics:');
    console.log(`  Pass Rate: ${(healthReport.metrics.overallPassRate * 100).toFixed(1)}% (threshold: ${(healthReport.thresholds.minimumPassRate * 100).toFixed(1)}%)`);
    console.log(`  Consistency: ${(healthReport.metrics.consistencyScore * 100).toFixed(1)}% (threshold: ${(healthReport.thresholds.minimumConsistency * 100).toFixed(1)}%)`);
    console.log(`  Avg Execution Time: ${healthReport.metrics.averageExecutionTime.toFixed(0)}ms (threshold: ${healthReport.thresholds.maximumExecutionTime}ms)`);
    console.log(`  Flakiness: ${(healthReport.metrics.flakiness * 100).toFixed(1)}% (threshold: ${(healthReport.thresholds.maximumFlakiness * 100).toFixed(1)}%)`);
    console.log(`  Total Tests: ${healthReport.metrics.totalTestsRun}`);
    
    // Performance regression
    if (Math.abs(healthReport.metrics.performanceRegression) > 5) {
      const direction = healthReport.metrics.performanceRegression > 0 ? 'slower' : 'faster';
      console.log(`  Performance: ${Math.abs(healthReport.metrics.performanceRegression).toFixed(1)}% ${direction} than baseline`);
    }
    
    // CI Status
    console.log('\n🚀 CI Status:');
    const ciIcon = healthReport.ciStatus.shouldPass ? '✅' : '❌';
    console.log(`  ${ciIcon} Should Pass CI: ${healthReport.ciStatus.shouldPass ? 'YES' : 'NO'}`);
    ...
    if (healthReport.issues.length > 0) {
      ...
      healthReport.issues.forEach(issue => {
        ...
        if (issue.affectedTests && issue.affectedTests.length > 0 && issue.affectedTests.length <= 3) {
          console.log(`    Affected: ${issue.affectedTests.join(', ')}`);
        } else if (issue.affectedTests && issue.affectedTests.length > 3) {
          console.log(`    Affected: ${issue.affectedTests.slice(0, 3).join(', ')} and ${issue.affectedTests.length - 3} more`);
        }
      });
    }
    ...
    if (healthReport.metrics.flakyTests.length > 0) {
      ...
      healthReport.metrics.flakyTests.slice(0, 5).forEach(test => {
        console.log(`  • ${test.testName}: ${(test.flakiness * 100).toFixed(1)}% flaky, ${(test.passRate * 100).toFixed(1)}% pass rate`);
      });
    }
    
    console.log('\n' + '='.repeat(60));
    console.log(`📄 Detailed report saved to: ${join(this.reportsDir, 'test-health-report.json')}`);
    console.log('='.repeat(60) + '\n');
  }
  ...
  saveHealthReport(healthReport) {
    try {
      ...
      const historicalPath = join(this.reportsDir, `test-health-report-${timestamp}.json`);
