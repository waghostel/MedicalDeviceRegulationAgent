...
function printHealthReport(report, options = {}) {
  ...
  // Key metrics
  console.log('\n📈 Metrics:');
  console.log(`  Pass Rate: ${formatPercentage(report.metrics.overallPassRate)}`);
  console.log(`  Consistency: ${formatPercentage(report.metrics.consistencyScore)}`);
  console.log(`  Execution Time: ${formatDuration(report.metrics.averageExecutionTime)}`);
  ...
  if (Math.abs(report.metrics.performanceRegression) > 1) {
    const direction = report.metrics.performanceRegression > 0 ? 'slower' : 'faster';
    console.log(`  Performance: ${Math.abs(report.metrics.performanceRegression).toFixed(1)}% ${direction}`);
  }
  
  // CI Status
  console.log('\n🚀 CI Status:');
  const ciIcon = report.ciStatus.shouldPass ? '✅' : '❌';
  console.log(`  ${ciIcon} Should Pass: ${report.ciStatus.shouldPass ? 'YES' : 'NO'}`);
  ...
  if (options.verbose) {
    ...
    if (report.issues.length > 0) {
      ...
      report.issues.forEach(issue => {
        ...
        console.log(`  ${severityIcon} ${issue.category.toUpperCase()}: ${issue.message}`);
        console.log(`     Recommendation: ${issue.recommendation}`);
        if (issue.affectedTests && issue.affectedTests.length > 0) {
          console.log(`     Affected Tests: ${issue.affectedTests.slice(0, 3).join(', ')}${issue.affectedTests.length > 3 ? '...' : ''}`);
        }
      });
    }
    ...
    if (report.metrics.slowTests.length > 0) {
      ...
      report.metrics.slowTests.slice(0, 10).forEach(test => {
        console.log(`  • ${test.testName}: ${formatDuration(test.averageTime)}`);
      });
    }
    ...
    if (report.metrics.flakyTests.length > 0) {
      ...
      report.metrics.flakyTests.slice(0, 10).forEach(test => {
        console.log(`  • ${test.testName}: ${formatPercentage(test.flakiness)} flaky, ${formatPercentage(test.passRate)} pass rate`);
      });
    }
    ...
    if (report.metrics.errorPatterns.length > 0) {
      ...
      report.metrics.errorPatterns.slice(0, 5).forEach(pattern => {
        console.log(`  • ${pattern.pattern} (${pattern.count} times, ${formatPercentage(pattern.percentage / 100)})`);
      });
    }
  }
...
function printHealthHistory(history, limit, options = {}) {
  ...
  console.log(`Showing last ${Math.min(limit || 10, history.length)} entries:\n`);
  
  // Table header
  console.log('Date/Time           | Pass Rate | Consistency | Exec Time | Flakiness');
  ...
  recentHistory.forEach(metrics => {
    ...
    console.log(`${date} | ${passRate} | ${consistency} | ${execTime} | ${flakiness}`);
  });
  ...
  if (options.verbose && recentHistory.length > 0) {
    ...
    console.log(`  Memory Usage: ${formatBytes(latest.memoryUsageAverage * 1024 * 1024)}`);
...
function printThresholds(thresholds, options = {}) {
  ...
  console.log('\n⚙️  Health Thresholds');
  console.log('='.repeat(50));
  console.log(`Minimum Pass Rate: ${formatPercentage(thresholds.minimumPassRate)}`);
  console.log(`Maximum Execution Time: ${formatDuration(thresholds.maximumExecutionTime)}`);
  console.log(`Maximum Test Time: ${formatDuration(thresholds.maximumTestTime)}`);
  console.log(`Minimum Consistency: ${formatPercentage(thresholds.minimumConsistency)}`);
  console.log(`Maximum Flakiness: ${formatPercentage(thresholds.maximumFlakiness)}`);
  console.log(`Maximum Regression: ${thresholds.maximumRegressionPercentage}%`);
  console.log(`Memory Threshold: ${formatBytes(thresholds.memoryThreshold * 1024 * 1024)}`);
}
...
module.exports = { main, printHealthReport, printHealthHistory, printThresholds };