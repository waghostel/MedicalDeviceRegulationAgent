...
export class CompatibilityLayerManager {
  ...
  async getData<T>(
    ...
    try {
      ...
      if (this.config.debugMode) {
        console.log(
          `[CompatibilityLayer] Using ${useRealData ? 'real' : 'mock'} data for ${dataType}`
        );
      }
      ...
      console.error(
        `[CompatibilityLayer] Error getting ${dataType} data:`,
        error
      );
  ...
  private shouldUseRealData(
    ...
    const databaseFlag = this.flagManager.evaluateFlag(
      'enable_real_database',
      context
    );
   ...
   */
  private async getRealDataWithFallback<T>(
    dataType: string,
    params?: any
  ): Promise<T> {
    ...
    try {
      ...
      if (
        this.config.errorHandling === ErrorHandlingStrategy.FALLBACK_TO_MOCK
      ) {
        console.warn(
          `[CompatibilityLayer] Falling back to mock data for ${dataType}:`,
          error
        );
    ...
    switch (this.config.errorHandling) {
      ...
      case ErrorHandlingStrategy.FALLBACK_TO_MOCK:
        if (this.config.fallbackToMock) {
          console.warn(
            `[CompatibilityLayer] Falling back to mock data due to error:`,
            error
          );
          this.metrics.fallbackCount++;
          return await this.getMockData(dataType, params);
  ...
  private updateMetrics(responseTime: number, success: boolean): void {
    ...
    const totalRequests =
      this.metrics.mockDataUsage + this.metrics.realDataUsage;
    ...
    const successfulAttempts =
      totalRequests - this.metrics.errorCount + (success ? 1 : 0);
...
class ProjectDataAdapter implements DataAdapter<Project[]> {
  ...
  validateData(data: Project[]): boolean {
    return (
      Array.isArray(data) &&
      data.every((project) => project.id && project.name && project.status)
    );
  }
}
 ...
 * Classification Data Adapter
 */
class ClassificationDataAdapter
  implements DataAdapter<DeviceClassification | null>
{
  async getMockData(params?: any): Promise<DeviceClassification | null> {
    const { generateMockDeviceClassification } = await import(
  ...
  async getRealData(params?: any): Promise<DeviceClassification | null> {
    ...
    const response = await fetch(
      `/api/projects/${params.projectId}/classification`
    );
...
class PredicateDataAdapter implements DataAdapter<PredicateDevice[]> {
  async getMockData(params?: any): Promise<PredicateDevice[]> {
    const { generateMockPredicateDevices } = await import(
      '../mock-data/generators'
    );
...
class InteractionDataAdapter implements DataAdapter<AgentInteraction[]> {
  async getMockData(params?: any): Promise<AgentInteraction[]> {
    const { generateMockAgentInteraction } = await import(
      '../mock-data/generators'
    );
