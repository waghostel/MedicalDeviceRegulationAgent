...
>(
  (
    ...
    const { announce, liveRegionProps, announcement } =
    ...
    }, [announce, announceOnOpen, announceOnClose]);

    // Auto-focus first input when dialog opens
    React.useEffect(() => {
      if (customFocusTarget?.current) {
        const timer = setTimeout(() => {
          customFocusTarget.current?.focus();
        }, 100);
      ...
      <DialogPortal>
        ...
        <DialogPrimitive.Content
          ...
          className={cn(
            ...
            'sm:rounded-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2',
            className
          )}
...
const DialogContentProvider: React.FC<{
  titleId: string;
  descriptionId: string;
  children: React.ReactNode;
}> = ({ titleId, descriptionId, children }) => (
