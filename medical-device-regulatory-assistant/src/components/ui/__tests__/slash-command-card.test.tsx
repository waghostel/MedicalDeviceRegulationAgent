...
describe('SlashCommandCard', () => {
  it('should render command information correctly', () => {
    ...
    expect(
      screen.getByText('Find similar predicate devices for 510(k) submissions')
    ).toBeInTheDocument();
  ...
  it('should call onExecute when clicked', () => {
    ...
    render(
      <SlashCommandCard command={mockCommand} onExecute={mockOnExecute} />
    );

    const card = screen
      .getByText('/predicate-search')
  ...
  it('should apply correct category styling', () => {
    ...
    expect(screen.getByText('classification')).toHaveClass(
      'bg-purple-100',
      'text-purple-800'
    );
    ...
    expect(screen.getByText('analysis')).toHaveClass(
      'bg-green-100',
      'text-green-800'
    );
    ...
    expect(screen.getByText('guidance')).toHaveClass(
      'bg-orange-100',
      'text-orange-800'
    );
  });
  ...
  it('should show disabled styling when disabled', () => {
    const { container } = render(
      <SlashCommandCard command={mockCommand} disabled={true} />
    );
