...
describe('CitationPanel Component', () => {
    ...
    describe('Basic Rendering', () => {
        it('renders citation panel with correct title and count', () => {
            renderWithProviders(<CitationPanel {...defaultProps} />, { session: mockSession });
        ...
        it('renders collapsible trigger button', () => {
            ...
            const triggerButton = screen.getByRole('button', { name: /citations & sources/i });
        ...
        it('shows collapsed state when isOpen is false', () => {
            renderWithProviders(
                <CitationPanel {...defaultProps} isOpen={false} />,
                { session: mockSession }
            );
    ...
    describe('Citation Display', () => {
        ...
        it('renders citation cards for each citation', () => {
            ...
            expect(screen.getByText('FDA 510(k) Database Entry K123456')).toBeInTheDocument();
            expect(screen.getByText('FDA Guidance Document - Class II Special Controls')).toBeInTheDocument();
            expect(screen.getByText('21 CFR 870.2300 - Cardiovascular Devices')).toBeInTheDocument();
        });

        it('shows empty state when no citations are provided', () => {
            renderWithProviders(
                <CitationPanel {...defaultProps} citations={[]} />,
                { session: mockSession }
            );
    ...
    describe('Control Buttons', () => {
        it('renders refresh button when onRefresh is provided', () => {
            ...
            const refreshButton = screen.getByRole('button', { name: /refresh citations/i });
        ...
        it('renders validate URLs button', () => {
            ...
            const validateButton = screen.getByRole('button', { name: /validate all urls/i });
        ...
        it('disables validate button during validation', async () => {
            const { validateSourceUrl } = require('../citation-utils');
            validateSourceUrl.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
    ...
    describe('Citation Search', () => {
        ...
        it('filters citations based on search input', async () => {
            ...
            await waitFor(() => {
                ...
                expect(screen.getByText('FDA Guidance Document - Class II Special Controls')).toBeInTheDocument();
                expect(screen.queryByText('FDA 510(k) Database Entry K123456')).not.toBeInTheDocument();
            });
        });
    });
    ...
    describe('Citation Interactions', () => {
        it('handles citation copy action', () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('Citation copied:', mockCitations[0].title);
        ...
        it('handles citation visit action', () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('Citation visited:', mockCitations[0].url);
            consoleSpy.mockRestore();
        });
    });
    ...
    describe('Error Handling', () => {
        it('handles missing callback props gracefully', () => {
            renderWithProviders(
                <CitationPanel citations={mockCitations} />,
                { session: mockSession }
            );
        ...
        it('handles URL validation errors gracefully', async () => {
            ...
            await waitFor(() => {
                expect(consoleSpy).toHaveBeenCalledWith('Failed to validate URLs:', expect.any(Error));
            });
        ...
        it('updates filtered citations when citations prop changes', () => {
            const { rerender } = renderWithProviders(
                <CitationPanel {...defaultProps} citations={mockCitations.slice(0, 1)} />,
                { session: mockSession }
            );
