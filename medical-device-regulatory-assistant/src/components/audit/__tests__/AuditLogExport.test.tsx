...
describe('AuditLogExport', () => {
  ...
  it('should render export dialog with default options', () => {
    render(
      <AuditLogExport interactions={mockInteractions} onClose={mockOnClose} />
    );
  ...
  it('should allow setting custom filename', () => {
    ...
    fireEvent.change(filenameInput, {
      target: { value: 'my-custom-audit-report' },
    });
  ...
  it('should toggle reasoning traces option', () => {
    ...
    const reasoningCheckbox = screen.getByLabelText(
  ...
  it('should toggle sources option', () => {
    ...
    const sourcesCheckbox = screen.getByLabelText(
  ...
  it('should display export summary', () => {
    ...
    expect(
      screen.getByText('• 1 interaction will be exported')
    ).toBeInTheDocument();
  ...
  it('should disable export button when no interactions', () => {
    ...
    expect(
      screen.getByText('• 0 interactions will be exported')
    ).toBeInTheDocument();
  });
  ...
  it('should handle multiple interactions', () => {
    ...
    render(
      <AuditLogExport
        interactions={multipleInteractions}
        onClose={mockOnClose}
      />
    );

    expect(
      screen.getByText('• 2 interactions will be exported')
    ).toBeInTheDocument();
  });
  ...
  it('should generate correct filename with timestamp', () => {
    ...
    expect(placeholder).toMatch(
      /audit-trail_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}\.pdf/
    );
  });
