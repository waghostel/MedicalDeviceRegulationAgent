...
jest.mock('../AuditLogFilters', () => ({
  AuditLogFilters: ({ onFiltersChange }: any) => (
    <div data-testid="audit-log-filters">
      <button
        onClick={() => onFiltersChange({ agentAction: 'predicate-search' })}
      >
        Apply Filter
      </button>
    </div>
...
describe('AuditLogPage', () => {
  ...
  it('should render audit trail content after loading', async () => {
    ...
    expect(
      screen.getByPlaceholderText(
        'Search interactions, reasoning, or input data...'
      )
    ).toBeInTheDocument();
  });
  ...
  it('should filter interactions based on search term', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('No interactions found matching your criteria.')
      ).toBeInTheDocument();
    });
  });
