...
export function ComplianceDashboard({ projectId }: ComplianceDashboardProps) {
  ...
  const verifyIntegrity = async () => {
    try {
      ...
      toast({
        title: result.is_valid ? "Integrity Verified" : "Integrity Issues Found",
  ...
  const getComplianceStatus = (score: number) => {
    if (score >= 0.95) return { status: 'excellent', color: 'text-green-600', icon: CheckCircle };
    if (score >= 0.80) return { status: 'good', color: 'text-yellow-600', icon: AlertTriangle };
  ...
  if (!complianceReport) {
    return (
      <Alert>
        ...
        <AlertDescription>
          No compliance data available. Please ensure audit trail data exists for this project.
        </AlertDescription>
      </Alert>
    );
  }
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                {Math.round(complianceReport.compliance_metrics.average_confidence * 100)}%
              </p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                {new Date(complianceReport.report_metadata.generated_at).toLocaleDateString()}
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {Object.entries(complianceReport.regulatory_compliance).map(([requirement, status]) => (
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm">
                  {Math.round(complianceReport.compliance_metrics.reasoning_completeness * 100)}%
                </span>
              </div>
              <Progress value={complianceReport.compliance_metrics.reasoning_completeness * 100} />
            </div>
            
            <div>
              <div className="flex justify-between mb-2">
                <span className="text-sm font-medium">Citation Completeness</span>
                <span className="text-sm">
                  {Math.round(complianceReport.compliance_metrics.citation_completeness * 100)}%
                </span>
              </div>
              <Progress value={complianceReport.compliance_metrics.citation_completeness * 100} />
            </div>
            
            <div>
              <div className="flex justify-between mb-2">
                <span className="text-sm font-medium">Confidence Score Coverage</span>
                <span className="text-sm">
                  {Math.round(complianceReport.compliance_metrics.confidence_score_coverage * 100)}%
                </span>
              </div>
              <Progress value={complianceReport.compliance_metrics.confidence_score_coverage * 100} />
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          {integrityResult ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">
                    {integrityResult.is_valid ? 'Integrity Verified' : 'Integrity Issues Detected'}
                  </span>
                </div>
                <Badge variant={integrityResult.is_valid ? "default" : "destructive"}>
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  <p className="font-medium">Total Entries</p>
                  <p className="text-muted-foreground">{integrityResult.total_entries}</p>
                </div>
                <div>
                  <p className="font-medium">Verified</p>
                  <p className="text-muted-foreground">{integrityResult.verified_entries}</p>
                </div>
                <div>
                  <p className="font-medium">Tampered</p>
                  <p className="text-muted-foreground">{integrityResult.tampered_entries.length}</p>
                </div>
              </div>
              
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Clock className="h-3 w-3" />
                Last verified: {new Date(integrityResult.verification_timestamp).toLocaleString()}
                <span className="ml-2">Algorithm: {integrityResult.hash_algorithm}</span>
              </div>
              ...
              {integrityResult.tampered_entries.length > 0 && (
                <Alert variant="destructive">
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    {integrityResult.tampered_entries.length} entries show signs of tampering. 
                    Entry IDs: {integrityResult.tampered_entries.join(', ')}
                  </AlertDescription>
                </Alert>
              )}
            </div>
          ) : (
            <p className="text-muted-foreground">Click "Verify Now" to check audit trail integrity</p>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2">
            {Object.entries(complianceReport.compliance_metrics.action_distribution).map(([action, count]) => (
