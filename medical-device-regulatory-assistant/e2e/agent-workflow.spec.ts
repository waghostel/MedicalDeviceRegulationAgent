...
test.describe('Agent Workflow and AI Interactions', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/agent/chat', async (route) => {
      ...
      if (
        message.includes('/predicate-search') ||
...
Would you like me to perform a detailed comparison with any of these predicates?`,
        ...
        message.includes('/classify-device') ||
...
Would you like me to search for predicate devices or provide more details about the regulatory requirements?`,
        ...
        message.includes('/find-guidance') ||
...
Would you like me to provide more details about any of these guidance documents or search for additional ones?`,
          ...
          sources: [
            {
              title:
                'Clinical Evaluation of Electrocardiographs - FDA Guidance',
...
What would you like help with today?`,
  ...
  test('Complete agent conversation flow with mock AI responses', async ({
    page,
  }) => {
    ...
    await page.fill(
      ...
      'Hello, I need help with my medical device'
    );
    ...
    await page.fill(
      '[data-testid="chat-input"]',
      '/classify-device cardiac monitoring device for continuous rhythm analysis'
    );
    await page.press('[data-testid="chat-input"]', 'Enter');

    // Wait for classification response
    await expect(
      page.locator('[data-testid="agent-message"]').last()
    ).toBeVisible({ timeout: 10000 });

    // Verify classification content
    const classificationResponse = page
  ...
  test('Slash command functionality and quick action buttons', async ({
    page,
  }) => {
    await page.goto('/projects/test-project/agent');
    ...
    const commandSuggestions = page.locator(
    ...
    await page.fill(
      '[data-testid="chat-input"]',
      '/predicate-search cardiac monitor continuous monitoring'
    );
    await page.press('[data-testid="chat-input"]', 'Enter');
  ...
  test('Citation panel updates and external link navigation', async ({
    page,
  }) => {
    await page.goto('/projects/test-project/agent');
    ...
    if (await citationPanel.isVisible()) {
      ...
      if (await expandButton.isVisible()) {
        ...
        await expect(
          page.locator('[data-testid="citation-details"]')
        ).toBeVisible();
    ...
    const messageWithCitations = page
      .locator('[data-testid="agent-message"]')
      .last();
    const inlineCitations = messageWithCitations.locator(
  ...
  test('File upload and document processing workflows', async ({ page }) => {
    ...
    if (await fileUploadButton.isVisible()) {
      ...
      const testFileContent =
        'Test document content for medical device specification';
      ...
      if (await analyzeButton.isVisible()) {
        ...
        await expect(
          page.locator('[data-testid="document-analysis"]')
        ).toBeVisible({ timeout: 15000 });

        // Verify analysis content
        const analysisResult = page.locator(
  ...
  test('Conversation history persistence and context maintenance', async ({
    page,
  }) => {
    await page.goto('/projects/test-project/agent');
    ...
    await page.fill(
      '[data-testid="chat-input"]',
      'I have a cardiac monitoring device'
    );
    await page.press('[data-testid="chat-input"]', 'Enter');
    ...
    // Wait for classification response
    await expect(
      page.locator('[data-testid="agent-message"]').nth(1)
    ).toBeVisible({ timeout: 10000 });
    ...
    await page.fill(
      '[data-testid="chat-input"]',
      'What predicates would work for this device?'
    );
    await page.press('[data-testid="chat-input"]', 'Enter');

    // Wait for contextual response
    await expect(
      page.locator('[data-testid="agent-message"]').nth(2)
    ).toBeVisible({ timeout: 10000 });
  ...
  test('Agent performance and response time testing', async ({ page }) => {
    ...
    const testQueries = [
      ...
      {
        query: '/predicate-search cardiac monitor',
        expectedTime: 8000,
    ...
    for (const testQuery of testQueries) {
      ...
      await expect(
        page.locator('[data-testid="agent-message"]').last()
      ).toBeVisible({ timeout: testQuery.expectedTime });
    ...
    await expect(page.locator('[data-testid="agent-message"]')).toHaveCount(
      testQueries.length + concurrentQueries.length,
      { timeout: 15000 }
    );
