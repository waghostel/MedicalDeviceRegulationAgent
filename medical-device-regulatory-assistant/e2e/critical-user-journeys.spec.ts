...
test.describe('Critical User Journeys', () => {
  ...
  test('Complete 510(k) Predicate Search Workflow', async ({ page }) => {
    ...
    await page.fill('[data-testid="project-description"]', 'Continuous cardiac monitoring device');
    await page.fill('[data-testid="device-type"]', 'Class II Medical Device');
    await page.fill('[data-testid="intended-use"]', 'Continuous monitoring of cardiac rhythm');
    ...
    await page.fill('[data-testid="chat-input"]', '/predicate-search cardiac monitor continuous rhythm');
    await page.press('[data-testid="chat-input"]', 'Enter');
    
    // 7. Wait for search results
    await expect(page.locator('[data-testid="predicate-results"]')).toBeVisible({ timeout: 30000 });
  ...
  test('Device Classification Workflow', async ({ page }) => {
    ...
    await page.fill('[data-testid="intended-use"]', 'Computer-assisted surgical procedures');
    await page.click('[data-testid="create-project-submit"]');
    ...
    await page.fill('[data-testid="chat-input"]', '/classify-device robotic surgical system computer-assisted');
    await page.press('[data-testid="chat-input"]', 'Enter');
    
    // 4. Verify classification results
    await expect(page.locator('[data-testid="device-class"]')).toBeVisible({ timeout: 20000 });
    await expect(page.locator('[data-testid="product-code"]')).toBeVisible();
    await expect(page.locator('[data-testid="regulatory-pathway"]')).toBeVisible();
    
    // 5. Verify confidence score
    const confidenceScore = await page.locator('[data-testid="classification-confidence"]').textContent();
  ...
  test('FDA Guidance Document Search', async ({ page }) => {
    ...
    await page.fill('[data-testid="chat-input"]', '/find-guidance cybersecurity medical devices');
    await page.press('[data-testid="chat-input"]', 'Enter');
    
    // 2. Verify guidance results
    await expect(page.locator('[data-testid="guidance-results"]')).toBeVisible({ timeout: 15000 });
  ...
  test('Real-time Dashboard Updates', async ({ page }) => {
    ...
    await page.click('[data-testid="agent-workflow-button"]');
    await page.fill('[data-testid="chat-input"]', '/classify-device test device');
    await page.press('[data-testid="chat-input"]', 'Enter');
    
    // 3. Wait for completion and return to dashboard
    await page.waitForSelector('[data-testid="classification-complete"]', { timeout: 30000 });
  ...
  test('Error Handling and Recovery', async ({ page }) => {
    ...
    await page.fill('[data-testid="chat-input"]', '/predicate-search valid device');
    await page.press('[data-testid="chat-input"]', 'Enter');
