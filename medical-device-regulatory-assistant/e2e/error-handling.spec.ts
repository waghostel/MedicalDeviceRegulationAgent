...
test.describe('Error Handling and Edge Cases', () => {
    ...
    test.describe('Network Failure Scenarios', () => {
        ...
        test('should handle offline functionality', async ({ page, context }) => {
            ...
            await expect(page.locator('[data-testid="new-project-button"]')).toBeDisabled();

            // Should show cached data if available
            await expect(page.locator('[data-testid="cached-projects"]')).toBeVisible();
            ...
            await expect(page.locator('[data-testid="new-project-button"]')).toBeEnabled();
        });
    });
    ...
    test.describe('Form Validation and Error States', () => {
        ...
        test('should handle server-side validation errors', async ({ page }) => {
            ...
            await page.fill('[data-testid="project-description-input"]', 'A'.repeat(1000));
            await page.click('[data-testid="create-project-submit"]');

            await expect(page.locator('[data-testid="name-server-error"]')).toContainText('Project name already exists');
            await expect(page.locator('[data-testid="description-server-error"]')).toContainText('Description too long');
        });
    ...
    test.describe('Browser Refresh and State Recovery', () => {
        test('should recover form state after refresh', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="form-recovery-dialog"]')).toBeVisible();
            ...
            await expect(page.locator('[data-testid="project-name-input"]')).toHaveValue('Test Project');
            await expect(page.locator('[data-testid="project-description-input"]')).toHaveValue('Test Description');
        });

        test('should handle refresh during ongoing operations', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="operation-recovery"]')).toBeVisible();
            await expect(page.locator('[data-testid="check-operation-status"]')).toBeVisible();
        });

        test('should maintain agent conversation context after refresh', async ({ page }) => {
            await page.goto('/agent');
            ...
            await expect(page.locator('[data-testid="chat-message"]')).toContainText('Classify my device');
            await expect(page.locator('[data-testid="agent-response"]')).toBeVisible();
        });
    });

    test.describe('Concurrent User Sessions and Data Conflicts', () => {
        test('should handle concurrent project edits', async ({ page, context }) => {
            ...
            await page.fill('[data-testid="project-name-input"]', 'Updated by User 1');
            await page.click('[data-testid="save-project"]');

            // Second user tries to save conflicting changes
            await page2.fill('[data-testid="project-name-input"]', 'Updated by User 2');
            await page2.click('[data-testid="save-project"]');

            // Should show conflict resolution dialog
            await expect(page2.locator('[data-testid="conflict-dialog"]')).toBeVisible();
            await expect(page2.locator('[data-testid="conflict-options"]')).toBeVisible();
        ...
        test('should handle real-time updates from other users', async ({ page, context }) => {
            ...
            await page2.fill('[data-testid="project-name-input"]', 'New Project from User 2');
            ...
            await expect(page.locator('[data-testid="new-project-notification"]')).toBeVisible();
    ...
    test.describe('Agent and AI Error Scenarios', () => {
        test('should handle AI service unavailable', async ({ page }) => {
            ...
            await page.fill('[data-testid="chat-input"]', 'Help me classify my device');
            await page.click('[data-testid="send-button"]');

            await expect(page.locator('[data-testid="ai-service-error"]')).toContainText('AI service is currently unavailable');
        ...
        test('should handle malformed AI responses', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="response-error"]')).toContainText('Unable to process AI response');
        ...
        test('should handle streaming response interruption', async ({ page }) => {
            ...
            await page.route('**/api/agent/stream', async (route) => {
                const response = new Response(
                    new ReadableStream({
                        start(controller) {
                            controller.enqueue(new TextEncoder().encode('data: {"partial": "response"}\n\n'));
            ...
            await expect(page.locator('[data-testid="stream-interrupted"]')).toContainText('Response was interrupted');
    ...
    test.describe('Data Validation and Edge Cases', () => {
        ...
        test('should handle extremely long text inputs', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="text-truncated-notice"]')).toBeVisible();
        });
    });

    test.describe('Browser Compatibility and Edge Cases', () => {
        test('should handle unsupported browser features gracefully', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="polling-mode-notice"]')).toContainText('Using polling mode');
        });
        ...
        test('should handle localStorage unavailable', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="session-only-notice"]')).toContainText('Data will not persist');
        });
        ...
        test('should handle memory constraints', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="memory-optimization"]')).toBeVisible();
            await expect(page.locator('[data-testid="pagination-enabled"]')).toBeVisible();
        });
    });
});
