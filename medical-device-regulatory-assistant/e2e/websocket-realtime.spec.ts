...
test.describe('WebSocket Real-time Functionality', () => {
  ...
  test('WebSocket Connection Establishment and Authentication', async ({
    ...
    const connectionMessages = wsMessages.filter(
    ...
    const projectId = await page
      .locator('[data-testid="project-id"]')
  ...
  test('Real-time Project Updates via WebSocket', async ({ page, context }) => {
    ...
    const updateMessages = wsMessages.filter(
    ...
    await page.fill(
      ...
      '/classify-device real-time test device'
    );
    ...
    const agentMessages = wsMessages.filter(
      (msg) =>
    ...
    const secondPageMessages: WebSocketMessage[] = [];
    ...
    await page.fill(
      '[data-testid="chat-input"]',
      '/predicate-search real-time test'
    );
    await page.press('[data-testid="chat-input"]', 'Enter');
    ...
    const secondPageAgentMessages = secondPageMessages.filter(
      (msg) =>
        msg.type === 'agent_interaction' ||
        msg.type === 'predicate_search_completed'
    );
  ...
  test('WebSocket Connection Recovery and Reconnection', async ({ page }) => {
    ...
    expect(wsConnection).not.toBeNull();
    const initialConnectionMessages = wsMessages.filter(
      (msg) => msg.type === 'connection_established'
    );
    ...
    const manualReconnectionMessages = wsMessages.filter(
      (msg) => msg.type === 'connection_established'
    );
    ...
    await page.fill(
      '[data-testid="chat-input"]',
      '/classify-device resilience test'
    );
    await page.press('[data-testid="chat-input"]', 'Enter');
  ...
  test('WebSocket Message Queuing and Delivery', async ({ page }) => {
    ...
    const interactionHistory = page.locator(
  ...
  test('WebSocket Performance and Scalability', async ({ page, context }) => {
    ...
    console.log(
      `Sent ${totalMessages} messages across ${userCount} users in ${messagingTime}ms`
    );
    ...
    const crossUserUpdatePromises = userPages.map(
      async (userPage, userIndex) => {
  ...
  test('WebSocket Error Handling and Edge Cases', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="auth-error-banner"]')
    ).toBeVisible();
    ...
    console.log(
      `Sent ${rapidMessages.length} rapid messages in ${rapidSendTime}ms`
    );
