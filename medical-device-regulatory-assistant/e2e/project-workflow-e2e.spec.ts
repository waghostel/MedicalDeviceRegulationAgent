...
test.describe('Project Workflow End-to-End Tests', () => {
  ...
  test('Complete Project Creation Workflow - UI to Database', async ({ page }) => {
    ...
    const projectData = {
      ...
      description: 'Comprehensive end-to-end test for cardiac monitoring device with advanced features',
      deviceType: 'Class II Cardiac Monitor',
      intendedUse: 'Continuous monitoring of cardiac rhythm in ambulatory patients with real-time alerts',
      priority: 'high',
    ...
    const deletePromise = page.waitForResponse(response => 
      response.url().includes(`/api/projects/${projectId}`) && response.request().method() === 'DELETE'
    );
    ...
    const getDeletedResponse = await page.request.get(`/api/projects/${projectId}`, {
      headers: {
        'Authorization': 'Bearer mock-jwt-token'
      }
    });
  ...
  test('Real-time Updates and WebSocket Functionality', async ({ page, context }) => {
    ...
    await expect(secondPage.locator('[data-testid="chat-interface"]')).toBeVisible();

    // Wait for WebSocket connections to establish
    await page.waitForTimeout(2000);

    // Step 5: Trigger agent action that should send real-time updates
    await page.fill('[data-testid="chat-input"]', '/classify-device cardiac monitor');
    ...
    const classificationMessages = wsMessages.filter(msg => 
      msg.type === 'classification_completed' || msg.type === 'agent_interaction'
    );
    ...
    await expect(page.locator('[data-testid="classification-status"]')).toHaveText('Completed');

    // Verify second page also received updates
    await secondPage.click('[data-testid="dashboard-button"]');
    await expect(secondPage.locator('[data-testid="classification-status"]')).toHaveText('Completed');

    // Step 8: Test project update notifications
    await page.click('[data-testid="edit-project-button"]');
    await page.fill('[data-testid="project-description"]', 'Updated via WebSocket test');
    ...
    const updateMessages = secondWsMessages.filter(msg => msg.type === 'project_updated');
  ...
  test('Error Scenarios and Recovery Mechanisms', async ({ page }) => {
    ...
    await page.fill('[data-testid="project-description"]', 'Testing network error handling');
    await page.click('[data-testid="create-project-submit"]');
    ...
    await page.click('[data-testid="edit-project-button"]');
    await page.fill('[data-testid="project-description"]', 'Testing server error');
    await page.click('[data-testid="update-project-submit"]');
    ...
    const originalDescription = await page.locator('[data-testid="project-description"]').textContent();
    ...
    await expect(page.locator('[data-testid="project-description"]')).toContainText(newDescription);

    // Wait for error and rollback
    await expect(page.locator('[data-testid="error-toast"]')).toBeVisible();
    
    // Verify rollback to original value
    await expect(page.locator('[data-testid="project-description"]')).toContainText(originalDescription || '');
  ...
  test('Performance Tests for Large Datasets', async ({ page }) => {
    ...
    await page.fill('[data-testid="search-input"]', 'Performance Test Project 1');
    ...
    const concurrentPromises = projects.slice(0, 10).map(async (project, index) => {
      const updateData = {
    ...
    const exportResponse = await page.request.get('/api/projects/export/bulk', {
      ...
      params: {
        format: 'json',
        project_ids: projects.slice(0, 20).map(p => p.id).join(',')
      }
    });
  ...
  test('Concurrent User Operations', async ({ page, context }) => {
    ...
    const updatePromises = userContexts.map(async (userPage, index) => {
      ...
      await userPage.fill('[data-testid="project-description"]', 
        `Updated by ${user.name} at ${Date.now()}`);
      
      const response = await userPage.request.put(`/api/projects/${projectId}`, {
        headers: {
    ...
    const agentPromises = userContexts.map(async (userPage, index) => {
      ...
      await userPage.fill('[data-testid="chat-input"]', 
    ...
    for (const userPage of userContexts) {
      ...
      await expect(userPage.locator('[data-testid="project-title"]')).toHaveText(projectData.name);
      
      // Verify agent interactions are visible
      await userPage.click('[data-testid="agent-workflow-button"]');
      const interactions = userPage.locator('[data-testid="interaction-history"] [data-testid="interaction-item"]');
  ...
  test('Database Transaction Integrity', async ({ page }) => {
    ...
    await page.fill('[data-testid="chat-input"]', '/classify-device transaction test device');
    await page.press('[data-testid="chat-input"]', 'Enter');
    await page.waitForSelector('[data-testid="classification-result"]', { timeout: 30000 });

    // Trigger predicate search
    await page.fill('[data-testid="chat-input"]', '/predicate-search transaction test');
    await page.press('[data-testid="chat-input"]', 'Enter');
    ...
    const complexUpdateResponse = await page.request.put(`/api/projects/${rollbackProject.id}`, {
      headers: {
        'Authorization': 'Bearer mock-jwt-token',
        'Content-Type': 'application/json'
      },
    ...
    const verifyResponse = await page.request.get(`/api/projects/${rollbackProject.id}`, {
      headers: {
        'Authorization': 'Bearer mock-jwt-token'
      }
    });
