...
test.describe('Dashboard Navigation and Functionality', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route(
      '**/api/projects/test-project/classification',
      async (route) => {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
            ...
            confidence: 0.92,
            reasoning:
              'Based on intended use and technological characteristics',
    ...
    await page.route(
      '**/api/projects/test-project/predicates',
      async (route) => {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
  ...
  test('Navigation between Project Hub, Dashboard, and Agent Workflow pages', async ({
    page,
  }) => {
    ...
    await page.click(
      '[data-testid="project-card"]:first-child, [data-testid="view-project-button"]:first-child'
    );
    ...
    await page.click(
      '[data-testid="agent-workflow-button"], [data-testid="agent-tab"]'
    );
    ...
    await page.click(
      '[data-testid="dashboard-button"], [data-testid="dashboard-tab"]'
    );
    await expect(
      page.locator('[data-testid="project-dashboard"]')
    ).toBeVisible();
    ...
    await page.click(
      '[data-testid="projects-button"], [data-testid="back-to-projects"]'
    );
    await expect(page.locator('[data-testid="project-hub"]')).toBeVisible();
  ...
  test('Widget interactions and state persistence across page transitions', async ({
    page,
  }) => {
    ...
    const expandButton = page.locator(
      '[data-testid="expand-classification-widget"]'
    );
    ...
    await expect(
      page.locator('[data-testid="predicate-card"]:first-child')
  ...
  test('Sidebar navigation and quick actions toolbar functionality', async ({
    page,
  }) => {
    await page.goto('/projects/test-project');
    ...
    const quickActionsToolbar = page.locator(
      ...
      for (const action of quickActions) {
        ...
        if (await actionButton.isVisible()) {
          ...
          const closeButton = page.locator(
            '[data-testid="close-modal"], [data-testid="cancel-button"]'
          );
  ...
  test('Responsive layout changes on tablet and mobile viewports', async ({
    page,
  }) => {
    const viewports = [
    ...
    for (const viewport of viewports) {
      ...
      if (viewport.width <= 768) {
        ...
        const mobileMenu = page.locator(
          '[data-testid="mobile-menu"], [data-testid="hamburger-menu"]'
        );
        ...
        if (widgetCount >= 2) {
          ...
          if (firstBox && secondBox) {
            ...
            const verticalLayout =
              secondBox.y > firstBox.y + firstBox.height - 50;
