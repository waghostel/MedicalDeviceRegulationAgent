...
class TestRunner {
  ...
  async runCommand(command, cwd = process.cwd(), description = '') {
    return new Promise((resolve, reject) => {
      ...
      child.on('close', (code) => {
        ...
        if (code === 0) {
          this.log(`✅ ${description || command} completed in ${duration}ms`, COLORS.GREEN);
          resolve({ success: true, duration });
        } else {
          this.log(`❌ ${description || command} failed with code ${code}`, COLORS.RED);
          resolve({ success: false, duration });
        }
      });

      child.on('error', (error) => {
        this.log(`❌ Error running ${description || command}: ${error.message}`, COLORS.RED);
  ...
  async checkPrerequisites() {
    ...
    try {
      ...
      const backendProcess = spawn('poetry', ['run', 'uvicorn', 'main:app', '--host', '0.0.0.0', '--port', '8000'], {
  ...
  async runFrontendTests() {
    ...
    let result = await this.runCommand('pnpm install', process.cwd(), 'Installing frontend dependencies');
    if (!result.success) return false;

    // Type checking
    result = await this.runCommand('pnpm type-check', process.cwd(), 'TypeScript type checking');
    if (!result.success) return false;

    // Linting
    result = await this.runCommand('pnpm lint', process.cwd(), 'ESLint checking');
    if (!result.success) return false;

    // Unit tests with coverage
    const startTime = Date.now();
    result = await this.runCommand('pnpm test:coverage', process.cwd(), 'Frontend unit tests with coverage');
    ...
    try {
      ...
      if (fs.existsSync(coverageFile)) {
        ...
        this.log(`📊 Frontend coverage: ${this.results.frontend.coverage}%`, COLORS.BLUE);
      }
    } catch (error) {
  ...
  async runBackendTests() {
    ...
    // Install dependencies
    let result = await this.runCommand('poetry install', backendDir, 'Installing backend dependencies');
    if (!result.success) return false;

    // Linting
    result = await this.runCommand('poetry run black --check .', backendDir, 'Black code formatting check');
    if (!result.success) return false;

    result = await this.runCommand('poetry run isort --check-only .', backendDir, 'isort import sorting check');
    if (!result.success) return false;

    result = await this.runCommand('poetry run flake8 .', backendDir, 'Flake8 linting');
    if (!result.success) return false;

    // Type checking
    result = await this.runCommand('poetry run mypy .', backendDir, 'MyPy type checking');
    if (!result.success) return false;
    ...
    try {
      ...
      if (fs.existsSync(coverageFile)) {
        ...
        if (match) {
          ...
          this.log(`📊 Backend coverage: ${this.results.backend.coverage}%`, COLORS.BLUE);
        }
      }
    } catch (error) {
  ...
  async runE2ETests() {
    ...
    let result = await this.runCommand('pnpm exec playwright install', process.cwd(), 'Installing Playwright browsers');
    if (!result.success) return false;

    // Run E2E tests
    const startTime = Date.now();
    result = await this.runCommand('pnpm test:e2e', process.cwd(), 'End-to-end tests');
  ...
  generateReport() {
    ...
    Object.entries(this.results).forEach(([suite, result]) => {
      ...
      console.log(`│ ${suite.padEnd(15)} │ ${status.padEnd(7)} │ ${coverage.padEnd(8)} │ ${duration.padEnd(8)} │`);
    });
  ...
  async run() {
    try {
      this.log(`${COLORS.BOLD}🧪 Medical Device Regulatory Assistant - Comprehensive Test Suite${COLORS.RESET}`, COLORS.BLUE);
      ...
      if (allPassed) {
        ...
        this.log('\n❌ Some tests failed. Please review and fix issues.', COLORS.RED);
        process.exit(1);
      }
