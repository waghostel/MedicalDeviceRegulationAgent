...
async function runBasicHealthCheck() {
  ...
  try {
    ...
    const report = {
      ...
      react19Compatible: react19Compatible,
    ...
    const dashboardHtml = [
      ...
      '        .score { font-size: 2em; font-weight: bold; color: ' + (healthScore >= 80 ? 'green' : healthScore >= 60 ? 'orange' : 'red') + '; }',
      '    </style>',
      '</head>',
      '<body>',
    ...
    if (healthScore >= 80) {
      summary = '✅ SUCCESS: All tests healthy (Score: ' + healthScore + '/100)';
    } else if (healthScore >= 60) {
      summary = '⚠️ WARNING: Some issues detected (Score: ' + healthScore + '/100)';
    } else {
      summary = '❌ CRITICAL: Multiple issues detected (Score: ' + healthScore + '/100)';
      exitCode = 1;
    }
    ...
    if (process.env.GITHUB_STEP_SUMMARY) {
      const stepSummary = [
        ...
        '| Metric | Value | Status |',
        '|--------|-------|--------|',
        '| Execution Time | ' + executionTime + 'ms | ' + (executionTime < 20000 ? '✅' : '⚠️') + ' |',
        '| React 19 Compatible | ' + (react19Compatible ? 'Yes' : 'No') + ' | ' + (react19Compatible ? '✅' : '❌') + ' |',
        '| CI Environment | ' + (process.env.CI ? 'Yes' : 'No') + ' | ' + (process.env.CI ? '✅' : '⚠️') + ' |',
        '',
        '---',
