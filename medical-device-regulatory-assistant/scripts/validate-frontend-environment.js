...
class FrontendEnvironmentValidator {
    ...
    validateNodeEnvironment() {
        ...
        try {
            ...
            try {
                ...
                result.addWarning('npm not available', 'Consider installing npm as fallback package manager');
    ...
    validateDependencies() {
        ...
        try {
            ...
            for (const dep of REQUIRED_DEPENDENCIES) {
                if (dependencies[dep]) {
                    installedDeps.push({ name: dep, version: dependencies[dep], type: 'required' });
                } else {
            ...
            for (const dep of REQUIRED_DEV_DEPENDENCIES) {
                if (dependencies[dep]) {
                    installedDeps.push({ name: dep, version: dependencies[dep], type: 'dev' });
                } else {
            ...
            if (fs.existsSync(nodeModulesPath)) {
                ...
                result.addDetail('node_modules_is_directory', nodeModulesStats.isDirectory());
            } else {
                result.addWarning(
    ...
    validateEnvironmentVariables() {
        ...
        result.addDetail('loaded_env_files', envFiles.filter(f => 
    ...
    validateBuildTools() {
        ...
        try {
            // Check ESLint
            const eslintVersion = execSync('npx eslint --version', { encoding: 'utf8' }).trim();
        ...
        try {
            // Check Prettier
            const prettierVersion = execSync('npx prettier --version', { encoding: 'utf8' }).trim();
    ...
    generateSetupInstructions(results) {
        const instructions = [];
        instructions.push('# Medical Device Regulatory Assistant - Frontend Setup Instructions\n');
        ...
        instructions.push('3. Navigate to project directory: `cd medical-device-regulatory-assistant`');
        instructions.push('4. Install dependencies: `pnpm install`');
        instructions.push('5. Copy .env.example to .env.local and configure variables');
        ...
        instructions.push('Run the frontend environment validator again to confirm all issues are resolved:');
...
function main() {
    ...
    if (!isValid) {
        console.log('\n📋 Setup Instructions:');
        console.log('Run with --instructions flag to see detailed setup instructions');
