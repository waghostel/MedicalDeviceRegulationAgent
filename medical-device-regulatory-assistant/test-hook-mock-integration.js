...
try {
  const useToastMockPath = path.join(__dirname, 'src/lib/testing/use-toast-mock.ts');
  ...
  if (missingMethods.length === 0) {
    ...
    console.log('❌ Missing methods in useToast mock:', missingMethods.join(', '));
  }
  ...
  if (missingContextualMethods.length === 0) {
    ...
    console.log('❌ Missing contextual toast methods:', missingContextualMethods.join(', '));
  }
...
try {
  const enhancedFormMockPath = path.join(__dirname, 'src/lib/testing/enhanced-form-hook-mocks.ts');
  ...
  if (missingRHFMethods.length === 0) {
    ...
    console.log('❌ Missing React Hook Form methods:', missingRHFMethods.join(', '));
  }
...
try {
  const componentMockPath = path.join(__dirname, 'src/lib/testing/enhanced-form-component-mocks.ts');
  ...
  if (missingTestAttributes.length === 0) {
    ...
    console.log('❌ Missing test attributes:', missingTestAttributes.join(', '));
  }
...
try {
  ...
  if (jestSetupContent.includes('useFakeTimers') || jestSetupContent.includes('jest.useFakeTimers')) {
  ...
  if (enhancedFormMockContent.includes('mockUseAutoSave') && enhancedFormMockContent.includes('setTimeout')) {
  ...
  console.log('❌ Error validating localStorage and timer mocks:', error.message);
}
...
try {
  const mockRegistryPath = path.join(__dirname, 'src/lib/testing/MockRegistry.ts');
  const componentMockPath = path.join(__dirname, 'src/lib/testing/enhanced-form-component-mocks.ts');
...
try {
  ...
  if (useToastMockContent.includes('toastMockUtils') && useToastMockContent.includes('clear') && useToastMockContent.includes('resetMocks')) {
...
try {
  ...
  if (fs.existsSync(integrationTestDir)) {
    ...
    const hookMockTests = files.filter(file => file.includes('hook-mock') || file.includes('integration'));
  ...
  if (!integrationTestExists) {
    console.log('⚠️  Integration tests need to be created for hook mock system');
  }
...
console.log('- Requirement 2.1: useToast mock structure matches actual implementation');
console.log('- Requirement 2.2: Enhanced form hook chain is properly mocked');
console.log('- Requirement 2.5: localStorage and timer mocks support auto-save functionality');
console.log('');
console.log('🎯 Integration test validation complete!');
console.log('');
console.log('Next Steps:');
console.log('1. Run the actual integration tests with: pnpm test --testPathPattern=integration');
