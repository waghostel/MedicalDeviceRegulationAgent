...
} = require('./scripts/validate-frontend-environment.js');
...
function testFrontendValidator() {
  ...
  console.log(
    `   Result: ${nodeResult.isValid ? '✅ Valid' : '❌ Issues Found'}`
  );
  ...
  console.log(
    `   Result: ${pnpmResult.isValid ? '✅ Valid' : '❌ Issues Found'}`
  );
  ...
  console.log(
    `   Result: ${filesResult.isValid ? '✅ Valid' : '❌ Issues Found'}`
  );
  console.log(
    `   Required files found: ${filesResult.details.existing_required_files?.length || 0}`
  );
  console.log(
    `   Required files missing: ${filesResult.details.missing_required_files?.length || 0}`
  );
  console.log(
    `   Package.json valid: ${filesResult.details.package_json_valid}`
  );
  ...
  console.log(
    `   Result: ${depsResult.isValid ? '✅ Valid' : '❌ Issues Found'}`
  );
  console.log(
    `   Installed dependencies: ${depsResult.details.installed_dependencies?.length || 0}`
  );
  console.log(
    `   Missing required: ${depsResult.details.missing_required_dependencies?.length || 0}`
  );
  console.log(
    `   Missing dev: ${depsResult.details.missing_dev_dependencies?.length || 0}`
  );
  console.log(
    `   node_modules exists: ${depsResult.details.node_modules_exists}`
  );

  console.log('\n5. Testing Environment Variables Validation...');
  const envResult = validator.validateEnvironmentVariables();
  console.log(
    `   Result: ${envResult.isValid ? '✅ Valid' : '❌ Issues Found'}`
  );
  const envVars = envResult.details.environment_variables || {};
  const setVars = Object.values(envVars).filter((v) => v === 'set').length;
  console.log(
    `   Environment variables set: ${setVars}/${Object.keys(envVars).length}`
  );

  console.log('\n6. Testing Build Tools Validation...');
  const buildResult = validator.validateBuildTools();
  console.log(
    `   Result: ${buildResult.isValid ? '✅ Valid' : '❌ Issues Found'}`
  );
  console.log(
    `   TypeScript available: ${buildResult.details.typescript_available}`
  );
  ...
  console.log(
    `   Prettier available: ${buildResult.details.prettier_available}`
  );

  console.log('\n7. Testing Comprehensive Validation...');
  const allResults = validator.runComprehensiveValidation();
  const overallValid = Object.values(allResults).every(
    (result) => result.isValid
  );
  console.log(
    `   Overall Result: ${overallValid ? '✅ All Valid' : '❌ Issues Found'}`
  );
...
function main() {
  console.log(
    'Medical Device Regulatory Assistant - Frontend Environment Validator Test'
  );
  ...
  console.log('='.repeat(60));
  console.log(
    `Frontend Validator Test: ${validatorResult ? '✅ PASSED' : '❌ FAILED'}`
  );
  console.log(
    `Overall Test Result: ${validatorResult ? '✅ ALL TESTS PASSED' : '❌ SOME TESTS FAILED'}`
  );
